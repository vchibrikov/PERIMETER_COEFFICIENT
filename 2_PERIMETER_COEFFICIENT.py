...
directory_path = "path/to/your/input/images"
output_dir = "path/to/your/output/images"
output_dir_excel = "path/to/your/output/excel"
...
canny_lower_threshold = # Default Canny lower threshold
canny_upper_threshold = # Default Canny upper threshold
canny_lower_boundary =  # Lower boundary for Canny slider
canny_upper_boundary = # Upper boundary for Canny slider
kernel_size =         # Default kernel size
kernel_lower_boundary = # Lower boundary for kernel slider
kernel_upper_boundary = # Upper boundary for kernel slider
kernel_step =       # Step size for kernel slider
blur_threshold =      # Default blur threshold
blur_step =         # Step size for blur slider
blur_size =        # Default blur kernel size
blur_lower_boundary =  # Lower boundary for blur slider
blur_upper_boundary =  # Upper boundary for blur slider
perpendicular_gap =   # Gap between perpendiculars
directional_gap =     # Gap between directional points

def process_image(image, lower_threshold, upper_threshold, kernel_size, blur_ksize = blur_size):
    """Apply Canny, morphological closing, contour filling, and edge smoothing."""
    
    # Apply Gaussian blur to smooth edges before Canny
    smoothed = cv2.GaussianBlur(image, (blur_ksize, blur_ksize), 0)
    
    # Apply Canny edge detection
    edges = cv2.Canny(smoothed, int(lower_threshold), int(upper_threshold))
    ...
    smoothed_edges = cv2.medianBlur(closed_edges, blur_size)
...
def scan_from_centroid(filled_object, centroid, num_closest_points = 3):
    """Scan from centroid and find the 3 closest edge points, then return their average."""
    if centroid is None:
        return None
    ...
    # Scan the whole image to find all edge points
    for radius in range(1, max_radius):
        for angle in np.linspace(0, 2 * np.pi, 360):
            ...
            if 0 <= x < filled_object.shape[1] and 0 <= y < filled_object.shape[0]:
                if filled_object[y, x] == 0:  # Edge pixel found (black pixel)
                    distance = calculate_length(centroid, (x, y))  # Calculate the distance from centroid
...
def scan_perpendicular_from_centroid(filled_object, centroid, edge_point, gap = directional_gap):
    """Scan from the centroid in two perpendicular directions (clockwise and counterclockwise)
       with a n-pixel gap in both directions."""
    if centroid is None or edge_point is None:
        return None, None
    ...
    max_radius = min(filled_object.shape[0], filled_object.shape[1]) // 2  # Max radius based on image size

    # Scan along the perpendicular direction clockwise with gap
    ...
    # Scan along the perpendicular direction counterclockwise with a gap
...
def get_line_points(centroid, edge_point):
    ...
    dx = ex - cx
    dy = ey - cy
    steps = max(abs(dx), abs(dy))  # Number of steps in the line
...
def plot_perpendiculars(ax, red_line_coords, filled_object, gap=perpendicular_gap):
    ...
    perpendicular_lengths = []  # List to store lengths of each perpendicular
    ...
    for i in range(0, len(red_line_coords), gap):
        ...
        if perp_start and perp_end:
            ...
            perpendicular_lengths.append(length_perpendicular)  # Append each length
...
def plot_blue_perpendiculars(ax, blue_line_coords, filled_object, gap=perpendicular_gap):
    ...
    blue_perpendicular_lengths = []  # List to store lengths of each blue perpendicular
...
def save_perpendicular_lengths_to_excel(image_file, blue_lengths, red_lengths):
    ...
    ws = wb.active
    ws.append(["filename", "y_axis_length_px", "x_axis_length_px"])  # Add the header for both columns
...
def update(val, im_obj, ax, fig):
    ...
    edge_point = scan_from_centroid(processed_img, centroid)
    if edge_point:
...
for image_file in image_files:
    ...
    # Create an axis for the image (top 80% of the figure)
    ax = fig.add_subplot(111)
    im_obj = ax.imshow(filled_object, cmap='gray')  # Assuming ax is your axes object
    ax.axis('off')  # Remove axes to focus on the image
    ...
    slider_blur = Slider(ax_blur, 'Blur', blur_lower_boundary, blur_upper_boundary, valinit = blur_threshold, valstep = blur_step)
